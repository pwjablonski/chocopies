(async function(d, L, d3) {
  ...
  function drawData(total, sent, eaten) {
    ...
    sentDiv.innerHTML = '100%'; //Math.round(((sent+eaten)/total) *100) + '%'; //sent + eaten;
    //console.log( Math.round(((sent+eaten)/total) *100) ); 
...
jQuery.notify=function(options,content){(function($){var defaults={type:"shout",text:"Lorem Ipsum is simply dummy text of the printing and typesetting industry",close:!1,speed:"slow",timeout:9000};$("body").prepend($("<div></div>").attr("id","notify"));if(typeof(options)==="object")
options=$.extend(defaults,options);var methods={shout:function(content){$("#notify").removeClass($(this).attr("class")).addClass("shout");$("#notify").html("").html(content);$("#notify").slideDown(defaults.speed).delay(defaults.timeout).slideUp("slow")},error:function(content){$("#notify").removeClass($(this).attr("class")).addClass("error");$("#notify").html("").html(content);$("#notify").slideDown(defaults.speed).delay(defaults.timeout).slideUp("slow")},success:function(content){$("#notify").removeClass($(this).attr("class")).addClass("success");$("#notify").html("").html(content);$("#notify").slideDown(defaults.speed).delay(defaults.timeout).slideUp("slow")}};if(methods[options]){return methods[options].call(this,content)}else{$("#notify").removeClass($(this).attr("class")).addClass(options.type);$("#notify").html("").html(options.text);if(options.close){$("#notify").append("<span class=\"close\">[x]</span>");$("span.close").on("click",function(){$("#notify").dequeue()})}
